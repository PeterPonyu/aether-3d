...
volumetric analysis. We introduce \textbf{Aether3D}, an audit-safe
...
methodology. Aether3D builds on entropic optimal
...
Aether3D contributes three independent levers: (i)~a uniform
...
\begin{figure}[t]
  ...
  \caption{Virtual-slice holdout: 7-metric quartet for the two
  always-available baselines on a synthetic 5-slice stack with the
  middle slice held out. Source:
  \texttt{aether-3d/results/benchmark/synthetic\_holdout.json}.}
  \label{fig:holdout}
\end{figure}
...
\begin{figure}[t]
  ...
  on synthetic data. (a) Top-1 coupling accuracy. (b) Mean true-pair
  mass. Source: \texttt{aether-3d/results/benchmark/uot\_ablation.json}.}
  \label{fig:ablation}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{Scaling curve on synthetic stacks. (a) Runtime vs cells per
  slice. (b) Provenance per measurement (device + CUDA version).
...
Aether3D in its present synthetic-evaluation form is not a claim of
