...
volumetric analysis. We introduce \textbf{\textsc{Aether3D}}, a
benchmark contract that wraps six 3D-reconstruction adapters under a
single virtual-slice holdout protocol: two always-available baselines
(nearest-slice and linear-interpolation) and four external competitors
that are wired through a shared input/output schema and run only when
their upstream packages are installed. Per virtual depth we report an
eight-metric suite: Chamfer distance, coordinate RMSE, sliced 2D
Wasserstein, Moran's I agreement, domain ARI, domain NMI, cell-type
proportion Spearman, and Betti-0 topology stability. Flow-divergence and
velocity-anisotropy vector-field diagnostics ship in the same module but
are computed only when a velocity field is available, not for the NumPy
baselines reported here. An $(\alpha_{\text{spatial}}
\times \lambda_{\text{class}})$ ablation grid quantifies the contribution of each
component of the hybrid coupling cost, and a scaling
