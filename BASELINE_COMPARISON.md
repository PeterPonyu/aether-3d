...
**Baseline**: `../baselines/DeepSpatial-original/`
...
| `DeepSpatialModule`                         | `AetherFlowModule`                                     | Consistent "Flow" naming with LuminaST |
...
| `transport/` + `models/commons.py` + `git.py` | `flow/` + `models/{...}` (clean re-type, deduped)     | Shared clean implementation with LuminaST (or vendored) |
...
## Relationship to LuminaST
Both projects share the cleaned `flow/` primitives (velocity prediction, Linear/GVP/VP paths, ODE/SDE samplers, EMA). This deduplication is an explicit improvement over the original two repos that copied ~800 LOC of transport code.

**Audit sign-off schedule**: same milestones as LuminaST.

Together with the frozen `baselines/DeepSpatial-original/` tree, this document provides complete traceability for the Aether3D publication.
